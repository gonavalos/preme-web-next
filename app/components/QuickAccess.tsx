...
export default function QuickAccess() {
  return (
    <section className="relative z-20 -mt-7 max-w-4xl mx-auto px-4 sm:px-6">
      <div className="bg-white/95 backdrop-blur-xl rounded-2xl shadow-xl shadow-black/8 border border-gray-100/80 px-4 sm:px-8 py-5 sm:py-7 flex items-center justify-between gap-1 sm:gap-4">
        {items.map((item) => (
          <Link
            key={item.label}
            href={item.href}
            className="group flex flex-col sm:flex-row items-center gap-2 sm:gap-3 px-3 sm:px-5 py-3 sm:py-4 rounded-xl hover:bg-[#092f57]/5 transition-all flex-1 justify-center"
          >
